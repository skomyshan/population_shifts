...
- Reviewing the final output of the the models, including the confusion matrices and classification reports, it is clear that there is an issue with our model in its ability to predict population growth. Despite the fact that population has consistantly increased over the past 30 or so years due to recent world trends with regard to medical advancements, our model has a tendancy to predict a decrease in year over year population increase. For our best model, the initial machines leanring run, the model predicted an increase only 26% of the time with over a third of these predictions being false positives (with positive being the 0 indicator or a decrease in the year over year populaton percentage increase). All in all, the final accuracy score sits as .6143 which is lower than what we would have preferred, but the issues with the model are deep enough that it would involve serious reconstruction of the data and the data used.
